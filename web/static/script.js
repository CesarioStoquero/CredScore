...
function calcDTI() {
    ...
    if (dti === 0) {
        ...
        hint.textContent  = 'Informe sua dívida mensal para calcular';
    } else if (dti < 0.35) {
        badge.style.color = 'var(--green)';
        hint.textContent  = '✅ Saudável — abaixo de 35%';
    } else if (dti < 0.50) {
        badge.style.color = 'var(--amber)';
        hint.textContent  = '⚠️ Atenção — entre 35% e 50%';
    } else {
        badge.style.color = 'var(--red)';
        hint.textContent  = '❌ Alto risco — acima de 50%';
    }
}
...
async function runPrediction() {
    ...
    try {
        ...
        btn.disabled    = false;
        btn.textContent = '⚡ Verificar probabilidade de aprovação';
    }
}
...
function showPredictionResult(data) {
    ...
    var factorNames  = ['Score de crédito', 'Pontuação DTI', 'Empréstimos existentes', 'Garantia'];
...
async function calcEmi() {
    ...
    try {
        ...
        document.getElementById('emiError').textContent   = 'Erro: ' + e.message;
...
function showEmiResult(data) {
    ...
    document.getElementById('eEffTen').textContent   = data.months_saved > 0
        ? data.effective_tenure + ' meses (economiza ' + data.months_saved + ' meses)'
        : data.effective_tenure + ' meses';

    var methodLabels = { reducing: 'Método de saldo devedor', flat: 'Método de taxa fixa', compound: 'Método de juros compostos' };
    document.getElementById('emiMethodLabel').textContent = methodLabels[data.interest_type] || 'Método de saldo devedor';
    ...
    if (data.interest_type !== 'reducing') {
        prepayNote.textContent   = 'Observação: a economia com pagamento antecipado só é calculada no método de saldo devedor.';
...
syncEmiSlider(document.getElementById('eTen'),  'eTenDisplay',  false, 0, ' meses');
calcDTI();
calcEmi();