...
def _load_model():
    ...
    try:
        ...
        if not os.path.exists(model_path):
            _load_error = f"loan_model.pkl não encontrado em: {model_path}"
            return None, None, _load_error
        if not os.path.exists(scaler_path):
            _load_error = f"scaler.pkl não encontrado em: {scaler_path}"
            return None, None, _load_error
        _model  = joblib.load(model_path)
        _scaler = joblib.load(scaler_path)
        print(f"[CredScore] Model loaded from {model_path}")
        return _model, _scaler, None
    except Exception as e:
        _load_error = f"Falha ao carregar o modelo: {str(e)}"
        return None, None, _load_error
...
def _build_tips(meta, probability):
    ...
    if meta['credit_score'] < 600:
        tips.append('Seu score de crédito está abaixo de 600. Melhorá-lo para acima de 700 pode aumentar bastante as chances de aprovação.')
    if meta['dti_ratio'] > 0.5:
        tips.append('Seu índice dívida/renda está alto (acima de 50%). Tente quitar ou reduzir dívidas existentes antes de solicitar.')
    if meta['existing_loans'] > 3:
        tips.append('Ter muitos empréstimos existentes reduz as chances de aprovação. Tente quitar alguns antes de solicitar.')
    if meta['collateral_value'] == 0 and probability < 60:
        tips.append('Informar uma garantia pode melhorar bastante suas chances de aprovação.')
    if meta['employment_status'] == 'unemployed':
        tips.append('Instituições financeiras preferem renda estável. Considere solicitar após conseguir um emprego.')
    if not tips:
        tips.append('Seu perfil parece forte! Considere negociar uma taxa de juros menor.')
    return tips
...
def predict(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Requisição POST obrigatória.'}, status=405)
    ...
    try:
        ...
        return JsonResponse({'error': f'Falha ao realizar a previsão: {str(e)}'}, status=500)

    if probability >= 75:
        verdict, verdict_class = 'Provável aprovação', 'green'
    elif probability >= 50:
        verdict, verdict_class = 'Chance moderada', 'amber'
    elif probability >= 30:
        verdict, verdict_class = 'Baixa probabilidade', 'orange'
    else:
        verdict, verdict_class = 'Provável rejeição', 'red'
...
def emi(request):
    ...
    try:
        ...
        if principal <= 0:
            return JsonResponse({'error': 'O valor do empréstimo deve ser maior que 0.'}, status=400)
        if tenure_months <= 0:
            return JsonResponse({'error': 'O prazo deve ser maior que 0.'}, status=400)
        if annual_rate < 0:
            return JsonResponse({'error': 'A taxa de juros não pode ser negativa.'}, status=400)
        ...
        if interest_type == 'flat':
            ...
            interest_note  = 'Taxa fixa: os juros são calculados sobre o valor principal original durante todo o prazo do empréstimo. A taxa efetiva tende a ser aproximadamente o dobro da taxa fixa informada.'
            ...
            interest_note  = 'Juros compostos: os juros são calculados sobre o principal somado aos juros acumulados (capitalização mensal). Isso resulta em juros totais mais altos.'
            ...
            interest_note  = 'Saldo devedor: os juros são cobrados apenas sobre o saldo principal em aberto. Conforme o pagamento avança, os juros diminuem. É o método mais comum em bancos.'
        ...
        return JsonResponse({'error': f'Falha ao calcular a parcela: {str(e)}'}, status=500)